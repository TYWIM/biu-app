...
- **P3 移动端核心能力已完成**：后台播放、通知栏控制、蓝牙/音频焦点、网络适配均已实现并通过真机验证。
- 项目仍处于早期阶段，功能完整性、后台稳定性与文档都在持续完善中。
- 现阶段优先推进歌词增强（P4）、离线缓存（P5）等进阶功能。

## ✨ 特色功能
- 🎼 支持登录 Bilibili 并获取收藏夹、稍后再看、历史记录等信息
- 🎧 高品质音频播放，优先拉取更高码率音频流（如无损 Flac，192K/Hi-Res）
- 🎵 Android 原生播放桥接入（ExoPlayer），支持后台播放、通知栏控制、蓝牙/音频焦点
- 🧩 轻量界面，内置浅色和深色主题，同时可自定义部分主题样式，持续优化手机端操作体验
- 📶 移动端网络适配：弱网缓冲策略、省流模式、Wi-Fi/移动数据切换自动恢复
